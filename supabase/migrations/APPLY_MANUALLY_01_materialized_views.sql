-- ============================================================
-- MIGRATION 1: Materialized Views + RPCs
-- ============================================================
...
-- 2. Indexes
CREATE UNIQUE INDEX IF NOT EXISTS idx_draw_stats_turno_num ON draw_stats (turno, num);
CREATE INDEX IF NOT EXISTS idx_draw_stats_turno ON draw_stats (turno);
...
BEGIN
  ...
  FROM draw_stats ds WHERE ds.turno = p_turno ORDER BY ds.num;
END;
...
BEGIN
  ...
  FROM markov_transitions mt WHERE mt.turno = p_turno;
END;
$$ LANGUAGE plpgsql STABLE;
...
BEGIN
  ...
  FROM cooccurrence_matrix cm WHERE cm.turno = p_turno;
END;
$$ LANGUAGE plpgsql STABLE;
