-- =============================================================
-- FIX: verify_predictions_on_draw_insert()
-- The previous trigger used TEXT array operators on JSONB column.
-- This rewrite handles JSONB numeros correctly.
-- =============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Parse JSONB numeros: handle both object {"2":[],"3":[],"4":[],"r":""} and array [45,67]
    IF jsonb_typeof(pred_record.numeros) = 'object' THEN
      pred_numeros := pred_record.numeros;
    ELSIF jsonb_typeof(pred_record.numeros) = 'array' THEN
      -- Array could be [45,67] (flat numbers) or ['{"2":[],"3":[]}'] (stringified JSON)
      IF jsonb_array_length(pred_record.numeros) = 1
        ...
        END;
      ELSE
        -- Flat array of numbers: treat as 2-cifras only
