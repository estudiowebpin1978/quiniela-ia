-- CRITICAL FIX: Add temporal guard to prevent data leakage
-- The function now only sees draws BEFORE prediction_date
-- This makes it safe for both production (default = today) and backtesting (pass historical date)
-- Also fixes: all_draws now filters by date < prediction_date instead of using all history
...
BEGIN
  ...
  SELECT
    ...
    -- 3 cifras: combine predicted number with last draw's head
    (SELECT jsonb_agg(LPAD(n3::TEXT, 3, '0'))
     FROM (SELECT fs.numero * 100 + (SELECT MOD(numbers[1], 100) FROM draws WHERE turno = target_turno AND date < prediction_date ORDER BY date DESC LIMIT 1) AS n3
           WHERE user_tier IN ('premium', 'admin')
           UNION ALL
           SELECT (SELECT MOD(numbers[1], 100) FROM draws WHERE turno = target_turno AND date < prediction_date ORDER BY date DESC LIMIT 1) * 100 + fs.numero AS n3
           WHERE user_tier IN ('premium', 'admin')
    ) sub) AS prediccion_3cifras,
    -- 4 cifras: combine with last two numbers
    (SELECT jsonb_agg(LPAD(n4::TEXT, 4, '0'))
     FROM (SELECT fs.numero * 1000 + (SELECT MOD(numbers[1], 100) FROM draws WHERE turno = target_turno AND date < prediction_date ORDER BY date DESC LIMIT 1) * 100
           + (SELECT MOD(numbers[2], 100) FROM draws WHERE turno = target_turno AND date < prediction_date ORDER BY date DESC LIMIT 1) AS n4
           WHERE user_tier IN ('premium', 'admin')
    ) sub) AS prediccion_4cifras,
    -- Redoblona: companions from ANY position in historical draws
