...
export function computeFeatureMatrix(sequences: number[][]): FeatureMatrix {
  ...
  for (let num = 0; num < 100; num++) {
    ...
    for (let i = 0; i < drawCount; i++) {
      ...
      digitSumMap[i] = computeDigitSum(drawn);
      digitalRootArr[i] = digitalRoot(drawn);
    }
    ...
    features.consecutive_pairs = drawCount > 1 ? consecutivePairsCount / ((drawCount - 1) * 100) : 0;
    ...
    features.consecutive_trios = drawCount > 2 ? consecutiveTriosCount / ((drawCount - 2) * 1000) : 0;
