...
export function analizarCiclos(
  ...
  const appearing3 = new Set<number>();
  const appearing4 = new Set<number>();
  ...
  const buildCiclos = (
    ...
    ultimosIdxMap: Map<number, number>
  ): CicloItem[] => {
    ...
    for (let n = 0; n < max; n++) {
      ...
      const ult = ultimosIdxMap.get(n) ?? -1;
      const ultimoCiclo = ult === -1 ? cicloPromedio : ultIdxGlobal - ult;
  ...
  const ciclos2Cifras = buildCiclos(distancias2, '2cifras', ultimosIdx);
  const ciclos3Cifras = buildCiclos(distancias3, '3cifras', ultimosIdx3);
  const ciclos4Cifras = buildCiclos(distancias4, '4cifras', ultimosIdx4);
...
function detectarPatronSemanal(
  ...
  const confianza = sorteos.length > 0 ? Math.round((maxFreq / (sorteos.length / 7)) * 100) / 100 : 0;
...
function detectarPatronMensual(
  ...
  const maxFreq = diasOrdenados[0]?.[1] || 1;
  const confianza = sorteos.length > 0 ? Math.round((maxFreq / (sorteos.length / 30)) * 100) / 100 : 0;
