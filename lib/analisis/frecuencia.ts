...
export function analizarFrecuencia(
  ...
  const calcularTendencia = (freq: Map<number, number>, total: number): FrecuenciaItem[] => {
    ...
    return Array.from(freq.entries())
      .map(([num, freqAbs]) => ({
        ...
        tendencia: total > 0 && ultimos30.length > 0 ? (freqReciente.get(num) || 0) / ultimos30.length - (freqAbs / total) : 0,
  ...
  const maxFreq = freqArray.length > 0 ? Math.max(...freqArray) : 0;
  const distribucion = maxFreq > 0 ? Array(10).fill(0).map((_, i) => {
