...
export function entrenarRandomForest(
  ...
  // Calculate precision using full ensemble (not just first tree)
  let correctas = 0;
  for (let i = 0; i < datos.length; i++) {
    const pred = predecirRandomForest(rf, datos[i].features);
    if (pred.prediccion === datos[i].etiqueta) correctas++;
  }
...
function construirArbol(
  ...
  if (!mejorSplit) {
    const counts = new Map<number, number>();
    let bestLabel = datos[0]?.etiqueta ?? 0;
    let bestCount = 0;
    for (const d of datos) {
      const c = (counts.get(d.etiqueta) || 0) + 1;
      counts.set(d.etiqueta, c);
      if (c > bestCount) { bestCount = c; bestLabel = d.etiqueta; }
    }
    return {
      feature: null,
      threshold: null,
