...
const MAX_ORDER = 1; // Cap at 1 to avoid OOM (order=2 = 1M entries = 80MB+)

export function crearCadenaMarkov(orden: number = 1): CadenaMarkov {
  const effectiveOrder = Math.min(orden, MAX_ORDER);
  return {
    matrizTransicion: Array.from({ length: NUM_ESTADOS ** effectiveOrder }, () => 
...
function calcularDistribucionEstacionaria(matriz: number[][]): number[] {
  ...
  // Power iteration: multiply dist by matrix repeatedly until convergence
  for (let iter = 0; iter < 50; iter++) {
    const nuevaDist = Array(n).fill(0);
    for (let j = 0; j < n; j++) {
      for (let i = 0; i < n; i++) {
        nuevaDist[j] += dist[i] * matriz[i][j];
      }
    }
    // Check convergence
    let diff = 0;
    for (let i = 0; i < n; i++) diff += Math.abs(nuevaDist[i] - dist[i]);
    dist = nuevaDist;
    if (diff < 1e-10) break;
  }
