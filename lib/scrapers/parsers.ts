...
// ─── Fuente 5: NotiTimba (Fallback robusto) ─────────────────────────────────
// SSR table with last 10 cabezas per province/turno. ISO-8859-1 encoded.
const NOTITIMBA_TURNO_MAP: Record<string, string> = {
  Previa: "La previa",
  Primera: "El primero",
  Matutina: "Matutina",
...
export async function parseNotitimba(_fechaISO: string, _fechaUrl: string, turno: string): Promise<ScrapeResult | null> {
  try {
    const r = await fetch("https://notitimba.com/lots/", {
      headers: { "User-Agent": randomUA(), "Accept": "text/html", "Accept-Charset": "iso-8859-1" },
    ...
    if (html.includes("SE HA GENERADO UN ERROR")) return null

    // Find the section for "La Ciudad" within the current turno
    // The HTML has <td>La Ciudad</td> followed by 10 <td>NNNN</td> cells
    // Structure: after "TURNO: <span id="Utrn">..." there are tables per turno
    const turnoLabel = NOTITIMBA_TURNO_MAP[turno] || turno

    // Find all "La Ciudad" rows - each has 10 number cells after it
    // Pattern: >La Ciudad</td> or >La Ciudad</th> followed by td cells with 4-digit numbers
    const ciudadRx = /La\s*Ciudad\s*<\/t[hd]>\s*((?:<td[^>]*>\s*\d{4}\s*<\/td>\s*){1,10})/gi
    const match = ciudadRx.exec(html)
    if (!match) return null

    // Extract the 10 numbers from the td cells
    const numRx = /<td[^>]*>\s*(\d{4})\s*<\/td>/gi
    const chunk = match[1]
    const nums: number[] = []
    let mx: RegExpExecArray | null
    ...
    if (nums.length >= 5) {
      return { numbers: nums, source: "notitimba.com", cabezaMatch: null }
    }
  } catch (e) {
    logger.debug("[scraper] parseNotitimba failed", { error: String(e) })
  }
  return null
}

// ─── Fuente 6: Loterías Mundiales (Cabezas del día, 5 turnos) ───────────────
// SSR homepage shows today's cabezas for all 5 turnos, 20 numbers each.
// Numbers appear as <b>NNNN</b> grouped by turno columns.
export async function parseLoteriasMundiales(_fechaISO: string, _fechaUrl: string, turno: string): Promise<ScrapeResult | null> {
  try {
    const r = await fetch("https://loteriasmundiales.com.ar/Quinielas/ciudad", {
      headers: { "User-Agent": randomUA(), "Accept": "text/html" },
      signal: AbortSignal.timeout(10000)
    })
    if (!r.ok) return null
    ...
    // Only works for today's date (homepage shows today)
    const hoy = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Argentina/Buenos_Aires", year: "numeric", month: "2-digit", day: "2-digit" }).format()
    if (_fechaISO !== hoy) return null

    // The page has turno headers with times: 10:15, 11:30, 14:00 a 15:30, 17:30 a 19:30, 21:00 a 22:30
    // Each turno section has 20 numbers in <b> tags
    const TURNO_TIMES: Record<string, string[]> = {
      Previa: ["10:15"],
      Primera: ["11:30"],
      Matutina: ["14:00"],
      Vespertina: ["17:30"],
    ...
    const times = TURNO_TIMES[turno]
    if (!times) return null

    // Find the section with the turno time
    let sectionStart = -1
    ...
    // Extract 20 numbers from <b>NNNN</b> after the time marker
    const afterTime = html.slice(sectionStart, sectionStart + 8000)
    const numRx = /<b>\s*(\d{4})\s*<\/b>/gi
    const nums = extractNums(afterTime, numRx, 20)

    if (nums.length >= 5) {
      return { numbers: nums, source: "loteriasmundiales.com.ar", cabezaMatch: null }
    }
  } catch (e) {
    logger.debug("[scraper] parseLoteriasMundiales failed", { error: String(e) })
  }
  return null
}
