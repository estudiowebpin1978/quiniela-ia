-- ============================================================================
-- 05-rpc-backtest.sql: Backtest and verification RPCs (fully idempotent)
-- ============================================================================
...
BEGIN
  ...
  SELECT
    up.id, up.user_id, up.game_id, up.date, up.turno,
    up.numbers_2c, up.numbers_3c, up.numbers_4c,
    d.numbers[1] % 100, d.numbers[1] % 1000, d.numbers[1],
    (d.numbers[1] % 100 = ANY(up.numbers_2c)),
    (d.numbers[1] % 1000 = ANY(up.numbers_3c)),
    (d.numbers[1] = ANY(up.numbers_4c)),
    NOW()
...
CREATE OR REPLACE FUNCTION compute_backtest(
  ...
  test_date DATE, rank_2c INT,
  hit_top1 BOOLEAN, hit_top5 BOOLEAN, hit_top10 BOOLEAN, score NUMERIC
...
BEGIN
  ...
  WITH draws AS (
    ...
    WHERE game_id = v_game_id AND turno = p_turno
      AND array_length(numbers, 1) >= 20
    ORDER BY date DESC LIMIT p_window
  ),
  test_draws AS (
    SELECT date, numbers, ROW_NUMBER() OVER (ORDER BY date) AS rn FROM draws
  ),
  results AS (
    SELECT td.date AS test_date, td.numbers[1] % 100 AS actual_2c,
      es.numero, es.final_score,
  ...
  SELECT r.test_date, r.rank::INT AS rank_2c,
    (r.rank = 1) AS hit_top1, (r.rank <= 5) AS hit_top5, (r.rank <= 10) AS hit_top10,
    r.final_score
  FROM results r WHERE r.rank <= 10 AND r.numero = r.actual_2c
...
BEGIN
  ...
  FOR v_turno IN SELECT DISTINCT turno FROM draws WHERE game_id = v_game_id LOOP
    INSERT INTO backtest_results (
      game_id, turno, model_type, test_date, train_window_start, train_window_end,
      hit_at_1_2c, hit_at_5_2c, hit_at_10_2c, rank_2c, score_2c, roi_2c
    )
    SELECT v_game_id, v_turno, 'ensemble', d.date,
      d.date - INTERVAL '365 days', d.date - INTERVAL '1 day',
      (d.numbers[1] % 100 = (SELECT numero FROM get_ensemble_scores(v_turno, p_game_slug) LIMIT 1)),
      EXISTS (SELECT 1 FROM get_ensemble_scores(v_turno, p_game_slug) sub WHERE sub.numero = d.numbers[1] % 100 ORDER BY sub.final_score DESC LIMIT 5),
      EXISTS (SELECT 1 FROM get_ensemble_scores(v_turno, p_game_slug) sub WHERE sub.numero = d.numbers[1] % 100 ORDER BY sub.final_score DESC LIMIT 10),
      (SELECT RANK() OVER (ORDER BY sub.final_score DESC)::INT FROM get_ensemble_scores(v_turno, p_game_slug) sub WHERE sub.numero = d.numbers[1] % 100 LIMIT 1),
      (SELECT final_score FROM get_ensemble_scores(v_turno, p_game_slug) sub WHERE sub.numero = d.numbers[1] % 100 LIMIT 1),
      CASE WHEN d.numbers[1] % 100 = (SELECT numero FROM get_ensemble_scores(v_turno, p_game_slug) LIMIT 1) THEN 60 ELSE -10 END
    FROM draws d
    WHERE d.game_id = v_game_id AND d.turno = v_turno
      AND d.date = CURRENT_DATE - INTERVAL '1 day'
      AND NOT EXISTS (SELECT 1 FROM backtest_results br WHERE br.game_id = v_game_id AND br.turno = v_turno AND br.test_date = d.date AND br.model_type = 'ensemble')
    LIMIT 1;
...
CREATE OR REPLACE FUNCTION get_motor_accuracy(
  p_motor TEXT, p_turno TEXT, p_game_slug TEXT DEFAULT 'quiniela'
...
BEGIN
  ...
  INTO v_accuracy FROM motor_performance mp
  WHERE mp.motor = p_motor AND mp.turno = p_turno
