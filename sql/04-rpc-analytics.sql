-- ============================================================================
-- 04-rpc-analytics.sql: Analytics RPCs (fully idempotent)
-- Uses turn_analytics.fecha (NOT date)
-- ============================================================================
...
BEGIN
  ...
  WITH draws AS (
    ...
    FROM draws WHERE game_id = v_game_id AND array_length(numbers, 1) >= 20
  ),
  pivoted AS (
    SELECT date,
    ...
    FROM draws GROUP BY date HAVING COUNT(DISTINCT turno) >= 3
  ),
  transitions AS (
    SELECT previa AS from_previa, primera AS to_primera,
      LAG(primera) OVER (ORDER BY date) AS prev_primera
    FROM pivoted
  )
  SELECT jsonb_build_object(
    'transitions', COALESCE(
      (SELECT jsonb_agg(jsonb_build_object('from', from_previa, 'to', to_primera, 'count', 1))
       FROM transitions WHERE from_previa IS NOT NULL), '[]'::JSONB),
    'order', 2, 'computed_at', NOW()
  ) INTO v_result;
  RETURN v_result;
END;
$$;
...
BEGIN
  ...
  WITH draws AS (
    ...
    WHERE game_id = v_game_id AND array_length(numbers, 1) >= 20
  ...
  probs AS (SELECT cnt / GREATEST(tot, 1) AS p FROM freq CROSS JOIN total),
  entropy_calc AS (
    SELECT COALESCE(-SUM(p * LOG(2, p)), LOG(2, 100)) AS raw_entropy
    FROM probs WHERE p > 0
  )
  SELECT raw_entropy / LOG(2, 100), 'stable' INTO v_entropy, v_trend FROM entropy_calc;
...
BEGIN
  ...
  critical AS (
    SELECT jsonb_agg(n) AS critical_numbers FROM gap_stats
...
-- ============================================================================
-- 4. GET LATEST ANALYTICS (reads turn_analytics.fecha)
-- ============================================================================
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'entropy', jsonb_build_object(
      'value', ta.entropy_value, 'trend', ta.entropy_trend, 'alert', ta.entropy_alert),
    'survival', ta.survival_hazard,
    'markov', ta.markov_transitions,
    'genetic_weights', ta.genetic_weights,
    'composite_confidence', ta.composite_confidence,
    'fecha', ta.fecha, 'computed_at', ta.fecha_calculo
  ) INTO v_result
  FROM turn_analytics ta
  WHERE ta.game_id = v_game_id AND ta.turno = p_turno
  ORDER BY ta.fecha DESC LIMIT 1;
...
BEGIN
  ...
  INSERT INTO turn_analytics (
    ...
    markov_transitions, markov_order, composite_confidence
  ) VALUES (
    v_game_id, p_turno, v_today,
    (v_entropy->>'entropy_value')::NUMERIC,
    v_entropy->>'entropy_trend',
    (v_entropy->>'entropy_alert')::BOOLEAN,
    v_survival, v_survival->'critical_numbers',
    jsonb_build_object('transitions', '[]'::JSONB, 'order', 2),
    2, 0.5
  )
  ON CONFLICT (game_id, turno, fecha) DO UPDATE SET
