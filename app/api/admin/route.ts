...
export async function POST(req: NextRequest) {
  ...
  // Validate UUID to prevent SQL injection via PostgREST filters
  ...
  try {
    const r = await fetch(`${SB()}/rest/v1/user_profiles?id=eq.${userId}`, {
      method: "PATCH",
      headers: { "apikey": SK(), "Authorization": `Bearer ${SK()}`, "Content-Type": "application/json", "Prefer": "return=minimal" },
      body: JSON.stringify(update)
    })
    if (!r.ok) return NextResponse.json({ error: "Error actualizando usuario" }, { status: 500 })
    return NextResponse.json({ ok: true })
  } catch {
