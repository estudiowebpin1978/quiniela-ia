...
import { ADMIN_EMAILS, PLAN_DAYS } from "@/lib/config"
...
export async function POST(req: NextRequest) {
  ...
  // Mark transfer as approved FIRST (atomic: only if still pending)
  const { data: updateResult, error: transferUpdateError } = await supabase
    .from("pending_transfers")
  ...
  if (transferUpdateError || !updateResult || updateResult.length === 0) {
    return NextResponse.json({ error: "Transferencia ya fue procesada" }, { status: 409 })
  }

  // Only activate premium AFTER atomic status check succeeds
