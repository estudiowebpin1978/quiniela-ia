...
export async function POST(req: NextRequest) {
  ...
  const email=body?.payer?.email||body?.email||body?.customer?.email
  const status=body?.status||body?.payment_status
  if(!email||!["approved","paid","completed","success"].includes(String(status).toLowerCase()))
    return NextResponse.json({ok:false,reason:"not approved"})
  try{
