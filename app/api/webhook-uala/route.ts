...
export async function POST(req: NextRequest) {
  ...
  const userId = body.external_reference || verification.externalReference || null
  if (!userId) {
  ...
  const plan = AMOUNT_PLAN_MAP[amountStr] || "30_days"
  ...
  try {
    const { data: existing } = await supabase
      .from("webhook_logs")
      .select("id")
      .eq("payload->>orderId", String(orderId))
      .limit(1)

    if (Array.isArray(existing) && existing.length > 0) {
      logger.info("[webhook-uala] Order already processed, skipping", { orderId })
      return NextResponse.json({ ok: true })
    }
  } catch {
    // webhook_logs table may not exist — continue
  }
  ...
  // ── 13. Log for idempotency ──────────────────────────────────────────
  try {
    await supabase.from("webhook_logs").insert({
      source: "ualabis",
      payload: JSON.stringify({ orderId, status: verification.status, amount, plan }),
      user_id: safeUserId,
      created_at: new Date().toISOString(),
    })
  } catch {
    // webhook_logs may not exist — ignore
  }
