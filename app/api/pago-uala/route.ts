  ...
  try {
    ...
    const orderRes = await fetch("https://checkout.developers.ar.ua.la/v2/api/checkout", {
      ...
      body: JSON.stringify({
        ...
        external_reference: `quiniela_${Date.now()}`
      })
    })
    const orderData = await orderRes.json()
    if (!orderRes.ok) return NextResponse.json({ error: "Error orden", httpStatus: orderRes.status, detail: orderData }, { status: 500 })
    const link = orderData.links?.checkout || orderData.checkout_link || orderData.link || `https://checkout.ualabis.com.ar/order/${orderData.uuid}`
