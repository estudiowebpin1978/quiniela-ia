...
export default function LoginPage() {
  ...
  const go = async () => {
    ...
    try {
      ...
      // Guardar sesion con TODAS las keys posibles que Supabase puede buscar
      ...
      localStorage.setItem("sb-wazkylxgqckjfkcmfotl-auth-token", token);
      localStorage.setItem("supabase.auth.token", token);
      setOk("Bienvenido! Redirigiendo...");
      setTimeout(() => { window.location.replace("/predictions"); }, 800);
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .tab{flex:1;padding:9px;text-align:center;border-radius:8px;border:none;background:transparent;color:rgba(255,255,255,.35);font-size:13px;font-weight:500;cursor:pointer;font-family:inherit}
        ...
        input{width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:#fff;font-size:15px;padding:12px 14px;outline:none;font-family:inherit}
        input:focus{border-color:rgba(201,168,76,.5)}
        input::placeholder{color:rgba(255,255,255,.2)}
        .btn{width:100%;background:linear-gradient(135deg,#c9a84c,#9a6f28);color:#fff;border:none;border-radius:11px;font-size:15px;font-weight:700;padding:14px;cursor:pointer;margin-top:20px;font-family:inherit}
        .btn:disabled{opacity:.5;cursor:not-allowed}
        .err{background:rgba(239,68,68,.09);border:1px solid rgba(239,68,68,.2);border-radius:9px;padding:11px 13px;color:#fca5a5;font-size:13px;margin-top:12px}
        ...
        .uala{display:block;width:100%;margin-top:9px;padding:12px;text-align:center;background:rgba(99,102,241,.09);border:1px solid rgba(99,102,241,.22);border-radius:10px;color:#a5b4fc;font-size:12px;text-decoration:none}
