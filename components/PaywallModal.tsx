...
export default function PaywallModal({ open, onClose, userId }: Props) {
  ...
  async function handleCardPay(plan: string) {
    ...
    try {
      ...
      try { data = JSON.parse(text) } catch { throw new Error("Error inesperado del servidor. Intentá de nuevo.") }
      if (!res.ok) throw new Error((data.error as string) || "Error creando la orden")
      if (data.checkoutUrl) {
