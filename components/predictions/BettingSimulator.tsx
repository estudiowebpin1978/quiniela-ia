...
import { getQuinielaEntry, getQuinielaName, getQuinielaIcon } from "@/lib/utils/quinielaDictionary"
...
function HeroCabeza({
  ...
  const culturalEntry = getQuinielaEntry(suggestion.cabeza.numero)
      ...
      <div className="relative">
        ...
        <div className="flex items-end gap-4">
          <span className="text-5xl sm:text-6xl font-black text-white tracking-tighter leading-none">
            {culturalEntry.icon} {suggestion.cabeza.numero}
          </span>
          <div className="flex-1 min-w-0">
            <p className="text-xs text-gray-400 truncate mb-1">{culturalEntry.name}</p>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {visible.map((d, i) => {
          const cultural = getQuinielaEntry(d.numero)
          return (
          <div
            key={d.numero}
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-black text-white">{cultural.icon} {d.numero}</span>
            </div>
            <div className="text-[9px] text-gray-500 mb-2 truncate">{cultural.name}</div>
...
export default function BettingSimulator({
  ...
  const headSignificado = getQuinielaName(predictions[0]?.numero || suggestion.cabeza.numero)
  ...
  return (
    <div className="bg-slate-900/80 rounded-2xl border border-slate-800 overflow-hidden backdrop-blur-sm">
      ...
      <div className="p-4 space-y-4">
        {/* Hero Card */}
        <HeroCabeza suggestion={suggestion} />
